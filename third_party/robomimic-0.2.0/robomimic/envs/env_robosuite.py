...
import diffusion_policy_3d.common.camera_util as camera_util
...
class EnvRobosuite(EB.EnvBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        update_kwargs = dict(
            ...
            camera_depths=True, # by default, it is false. we change it to true
        )
    ...
    def get_observation(self, di=None):
        ...
        for cam_name, cam_height, cam_width in zip(self.env.camera_names, self.env.camera_heights, self.env.camera_widths):
            if f"{cam_name}_depth" in di:
                ...
                R, t = self.get_camera_extrinsic_matrix(cam_name) # we transform into woord coord
                # R = t = None
    ...
    def get_camera_extrinsic_matrix(self, camera_name):
        cam_id = self.env.sim.model.camera_name2id(camera_name)
        cam_pose = self.env.sim.model.cam_pos[cam_id] # (3,)
        cam_quat = self.env.sim.model.cam_quat[cam_id] # (4,)
        R = camera_util.quat2mat(cam_quat)
        t = cam_pose
        return R, t
