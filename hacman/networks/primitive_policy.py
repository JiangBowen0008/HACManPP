...
class PrimitivePolicyWithArgmaxQ(PrimitivePolicy):
    ...
    def get_action(self, obs):  
        assert self.model, "PrimitivePolicyWithArgmaxQ: needs to load a model."      
        ...
        with torch.no_grad():
            ...
            if self.model.actor.features_extractor.__class__.__name__ == 'PointCloudGlobalExtractor':
                ...
                action_dim = actions.shape[-1] // num_prims
                actions = actions.reshape(-1, num_prims, action_dim)

                # Get per point feature
                features = self.model.critic.features_extractor(preprocessed_obs, per_point_output=True)
                num_points = features.shape[-2]
                features = torch.repeat_interleave(features, num_prims, dim=0)

                # One hot encoding of prim index
                prim_encoding = torch.eye(num_prims).repeat((num_points, 1)).to(device)
...
class RandomLocation(PrimitivePolicy):
    def __init__(self) -> None:
        return
