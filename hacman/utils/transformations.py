...
def voxel_downsample(points, voxel_size):
    """
    Args:
        points: Point cloud as a tensor [N, C]
        voxel_size: The size of the voxel grid
    Return: 
        Downsampled point cloud index
    """
    # Round the point coordinates to discretize the point cloud into voxels
    # voxel_keys = np.round(points[:, :3] / voxel_size)
    voxel_keys = torch.div(points[:, :3], voxel_size, rounding_mode='floor').int()
    
    # Use these rounded coordinates as unique identifiers for the voxels
    _, unique_idx = np.unique(voxel_keys.detach().cpu().numpy(), axis=0, return_index=True)
    
    # Select one point from each voxel (the first one encountered)
    downsampled_points = points[unique_idx]
    # downsampled_voxels = voxel_keys[unique_idx] * voxel_size + voxel_size / 2
    # downsampled_points[:, :3] = downsampled_voxels
