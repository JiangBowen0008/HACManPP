...
class BinArena(Arena):
    ...
    def __init__(
            self,
            xml_filename="bin_arena_all_side_planes.xml",
            bin_pos=(0, 0, 0),
            bin_base_size=(0.45, 0.54, 0.107),
            bin_friction=(0.3, 0.005, 0.0001),
            bin_solref=(0.02, 1.),
            bin_solimp=(0.9, 0.95, 0.001), 
            bin_thickness=0.01,
            bin_height=0.8,
            table_base_size=(0.45, 0.54, 0.107),
            arena_rand_size=(0, 0, 0),
            obstacle_full_size=(0.45, 0.02, 0.07),
            hidden_walls="",
    ):
        ...
        self.obstacle_full_size = np.array(obstacle_full_size) if obstacle_full_size is not None else None
        ...
        self.bin_collisions = {}
        self.bin_planes = {}
        for name in ['bottom', 'left', 'right', 'front', 'back', 'obstacle']:
    ...
    def configure_location(self):
        ...
        # Obstacle
        if self.obstacle_full_size is not None:
            size = self.obstacle_full_size / 2.
            pos = np.array([0, 0, size[2]])
        else:
            size = np.array([0.01, 0.01, 0.01])
            pos = np.array([0, 0, -0.1])
        
        # if "R" in self.hidden_walls: pos = np.array([0, 0, -1])
        self.bin_visuals["obstacle"].set("size", array_to_string(size))
        self.bin_visuals["obstacle"].set("pos", array_to_string(pos))
        self.bin_collisions["obstacle"].set("size", array_to_string(size))
        self.bin_collisions["obstacle"].set("pos", array_to_string(pos))
        ...
        # Set table locations
        table_full_size = np.array([self.table_full_size[0], self.bin_full_size[1], 0.05])
