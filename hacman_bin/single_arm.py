...
import os
import copy
from hacman_bin.osc import OSC
from hacman_bin.panda_gripper import PandaGripper, PandaGripperNarrow
...
class SingleArm(Manipulator):
    ...
    def load_model(self):
        ...
        if self.has_gripper:
            ...
            elif self.gripper_type == 'panda_festo':
                self.gripper = PandaGripper(idn = self.idn)
            elif self.gripper_type == 'panda_narrow':
                self.gripper = PandaGripperNarrow(idn = self.idn)
            elif self.gripper_type == 'panda_festo_narrow':
                self.gripper = PandaGripper(idn = self.idn)
                # print('setting gripper width to 0.8')
                self.gripper.actuator[0].set("ctrlrange", "0.0 0.0324")
                self.gripper.actuator[1].set("ctrlrange", "-0.0324 0.0")
            else:
