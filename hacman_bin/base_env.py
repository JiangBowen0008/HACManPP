...
from hacman_bin.bin_arena import BinArena
from hacman_bin.double_bin_arena import DoubleBinArena
from hacman_bin.single_arm import SingleArm
...
class BaseEnv(ManipulationEnv):
    ...
    Args:
        ...
        table_base_size (3-tuple): x, y, and z dimensions of the table.
    ...
    def __init__(
        ...
        table_base_size=None, ## a little smaller so that no points outside bin is included    # (0.45, 0.54, 0.107),
        ...
        close_gripper=False,
        remove_walls=False,
        object_dataset="housekeep_all",     # {"cube", "acronym", "shapenet", "housekeep"}
        object_types=None,
        object_name="rubiks_cube_077_rubiks_cube_M",
        convex_decomposed=False,
        object_split=None,
        object_scale_range=None,
        object_size_limit=[0.05, 0.05],
        object_init_pose="planar", #{"planar", "fixed", "any"}
        obstacle_full_size=None,
        arena_type="bin",           # {"bin", "double_bin"}
        gripper_friction=3,
        **kwargs
    ):
        ...
        # Optional settings for the bin arena
        self.table_base_size = table_base_size
        self.arena_rand_size = arena_rand_size
        self.obstacle_full_size = obstacle_full_size
        self.bin_base_size = bin_base_size
        ...
        self.set_camera_names = set_camera_names
        self.camera_poses = camera_poses
        self.camera_quats = camera_quats
        ...
        if object_dataset == 'housekeep' or object_dataset == 'housekeep_all':
            self.object_sampler = HousekeepSampler(
                ...
                convex_decomposed=convex_decomposed,
        ...
        self.gripper_friction_min = gripper_friction
        self.gripper_friction_max = gripper_friction
        self.gripper_friction_val = gripper_friction
    ...
    def get_bin_centers(self):
        return self.model.mujoco_arena.get_bin_centers()
    
    def get_bin_height(self):
        return self.model.mujoco_arena.bin_height
    
    def get_bin_half_sizes(self):
        """
        Get half sizes of the bins
        """
        return self.model.mujoco_arena.get_bin_half_sizes()
    
    def get_table_full_size(self):
        return self.model.mujoco_arena.get_table_full_size()
    ...
    def _load_model(self):
        ...
        arena_kwargs = dict(
            bin_pos=self.table_offset,
            bin_friction=self.table_friction,
            bin_solref=self.table_solref,
            bin_solimp=self.table_solimp,
            bin_base_size=self.bin_base_size,
            bin_thickness=self.bin_thickness,
            bin_distance=self.bin_distance,
            obstacle_full_size=self.obstacle_full_size,
            table_base_size=self.table_base_size,
            arena_rand_size=self.arena_rand_size,
            hidden_walls="",
        )
        # Remove default kwargs
        arena_kwargs = {k: v for k, v in arena_kwargs.items() if v is not None}

        if self.arena_type == "double_bin":
            # arena_kwargs['bin_full_size'] = self.table_base_size + (np.random.rand(3)*2 - 1)*self.table_size_rand
            mujoco_arena = DoubleBinArena(**arena_kwargs)
        elif self.arena_type == "bin": 
            if self.remove_walls:
                arena_kwargs['bin_base_size'] = np.array([0.8, 0.8, 0.107])
                arena_kwargs['hidden_walls'] = "FBLR"
            mujoco_arena = BinArena(**arena_kwargs)
        if self.set_camera_names is not None:
            for i in range(len(self.set_camera_names)):
                mujoco_arena.set_camera(camera_name=self.set_camera_names[i], pos = self.camera_poses[i], quat = self.camera_quats[i])
        ...
        # First selects a random bin
        bin_centers = mujoco_arena.get_bin_centers()
        # bin_centers[:, 0] += 0.07
        bin_scales = mujoco_arena.get_bin_half_sizes()
        bin_idx = np.random.randint(len(bin_centers))
        bin_center, bin_scale = bin_centers[bin_idx], bin_scales[bin_idx]

        # Random lower bounds
        max_dimension = 0.1
        bin_lower_bound = -bin_scale[:2] + max_dimension/2 + bin_center[:2]
        bin_upper_bound = bin_scale[:2] - max_dimension/2 + bin_center[:2]
        max_obj_dim = np.linalg.norm(self.get_cube_size())
        bin_lower_bound = bin_center + 0.8* np.clip(-bin_scale + max_obj_dim, a_min = None, a_max = 0)# 0.75
        bin_upper_bound = bin_center + 0.8* np.clip( bin_scale  - max_obj_dim, a_min = 0, a_max = None)
        bin_lower_bound[1] = bin_center[1]
        bin_upper_bound[1] = bin_center[1]

        if self.object_init_pose == "planar":
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=self.cube,
                x_range=[bin_lower_bound[0], bin_upper_bound[0]],
                y_range=[bin_lower_bound[1], bin_upper_bound[1]],
                rotation=None,
                rotation_axis='z',
            ...
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=self.cube,
                x_range=[bin_centers[0][0], bin_centers[0][0]],
                y_range=[bin_centers[0][1], bin_centers[0][1]],
                rotation=0,
                rotation_axis='z',
                ensure_object_boundary_in_range=False,  # Problematic
                ensure_valid_placement=False,
        ...
        elif self.object_init_pose == "fixed_off_center":
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=self.cube,
                x_range=[0, 0],
...
if __name__ == "__main__":
    import pickle
    with open("temp_config.pk", 'rb') as f:
        kwargs = pickle.load(f)
    env = BaseEnv('Virtual', **kwargs)