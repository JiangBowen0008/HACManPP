...
def process_objs():
    ...
    for obj_dir in ['box_pink_M']:
        ...
        # obj_sub_path = os.path.join(dirname, obj_dir)
        obj_sub_path = os.path.join(processed_dirname)
        for obj_file in os.listdir(obj_sub_path):
            if obj_file != 'box_pink_M.obj':
                continue
            if obj_file[-4:] == '.obj':
                obj_path = os.path.join(obj_sub_path, obj_file)
                ...
                stl_file = obj_file + '.stl'
                if not os.path.exists(processed_dirname):
                    os.makedirs(processed_dirname)
