...
# [Note] Maybe we need to modify this 
class AdroitEnv:
    ...
    def __init__(self, env_name, test_image=False,
        ...
        #     'door-v0': ['top'],
        #     'pen-v0': ['vil_camera'],
        #     'relocate-v0': ['front', 'bird', 'agent', 'side'], # only this will be used
        #     # 'relocate-v0': ['cam1', 'cam2', 'cam3',],
        #     # 'relocate-v0': ['cam1',], # bottom
        #     # 'relocate-v0': ['cam2',], # bottom
        #     # 'relocate-v0': ['fixed',], # side (the hand will occlude the object)
        #     # 'relocate-v0': ['fixed',], 
        ...
        if env_feature_type == 'state':
            ...
            width = 256
            latent_dim = 512
            env = BasicAdroitEnv(env, cameras=self.camera_names,
            ...
            height = HEIGHT
            width = WIDTH
            latent_dim = height*width*len(self.camera_names)*num_frames
            # RRL class instance is environment wrapper...
            env = BasicAdroitEnv(env, cameras=self.camera_names,
                                 height=height, width=width, latent_dim=latent_dim, hybrid_state=True,
        ...
        number_channel = len(self.camera_names) * 3 * num_frames

        if env_feature_type == 'pixels':
            self._obs_spec = specs.BoundedArray(shape=(
                number_channel, WIDTH, HEIGHT), dtype='uint8', name='observation', minimum=0, maximum=255)
            ...
            self._obs_spec = specs.Array(shape=(
                512 * num_frames * len(self.camera_names),), dtype='float32', name='observation')  # TODO fix magic number
            self._obs_sensor_spec = specs.Array(
                shape=(self.obs_sensor_dim,), dtype='float32', name='observation_sensor')
        ...
        self.observation_space = spaces.Dict({
            'image': spaces.Box(
                low=0,
                high=1,
                shape=(number_channel, WIDTH, HEIGHT),
                dtype=np.float32
            ),
            'depth': spaces.Box(
                low=0,
        ...
        # print(f"successfully init {env_name}")
    ...
    # # [Debug]
    # def render(self):
    #     assert (self.observation is not None) and (self.render_camera in self.camera_names)
    #     print(self.observation.keys())
    #     rgb_key = f'{self.render_camera}_image'
    #     return self.observation['image'][rgb_key]

    # [Denug]
    def reset(self, enable_pcd=False):
        # pixels and sensor values
        print('reset in adroit')
        obs_all = self._env.reset(enable_pcd=enable_pcd) # call MuJoCo base class's reset()
        # obs_rgbd, obs_sensor = obs_all
    ...
    def step(self, action, enable_pcd=False):
        ...
        obs_all, reward, done, env_info = self._env.step(action, enable_pcd=enable_pcd)
    ...
    #     img = self.get_pixels_with_width_height(WIDTH, HEIGHT)
